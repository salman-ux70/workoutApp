...
class CircularProgressBar @JvmOverloads constructor(
    ...
    private val mPaint =
        Paint(Paint.ANTI_ALIAS_FLAG) // Allocate paint outside onDraw to avoid unnecessary object creation
    ...
    private fun drawOutlineArc(canvas: Canvas) {
        val diameter = min(mViewWidth.toDouble(), mViewHeight.toDouble()).toInt()
    ...
    private fun drawText(canvas: Canvas) {
        mPaint.textSize = (min(mViewWidth.toDouble(), mViewHeight.toDouble()) / 5f).toFloat()
    ...
    fun setProgress(progress: Int) {
        ...
        animator.setDuration(mAnimationDuration.toLong())
